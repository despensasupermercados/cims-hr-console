...
export function installAutoSend(deps) {
  ...
  // Manual on/off switch, controlled from the Keyman page toggle. Defaults OFF
  // (missing row / table => disabled), so nothing fires until Miguel activates it.
  async function isEnabled(env) {
    try { var r = await env.DB.prepare("SELECT v FROM app_setting WHERE k='auto_send_enabled'").first(); return !!(r && r.v === "true"); }
  ...
  async function runAutoSend(env, event) {
    ...
    if (!(await isEnabled(env))) return { skipped: "disabled" };   // toggle OFF -> do nothing at all
    await ensureLog(env);
    var DRY = false;                                                // toggle ON == live sending
    ...
    return { ran: true, sent: sent.length, alerts: alerts.length };
  }
