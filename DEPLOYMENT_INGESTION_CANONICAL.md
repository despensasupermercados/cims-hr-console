...
- Recognizer (`recognize(wb)`):
  - **CEL/RCI wide** if a sheet named `Export` has `PORT NAME` in row 3. Brand row 0 (`CEL`→Celebrity, `RCI`→Royal Caribbean); ship row 2; 7-col blocks from col 2 (`| PORT RANK "PORT NAME" ARRIVE DEPART TENDER`); berth date col 0 (`- Stop N` suffix → stop_seq). `is_sea` = RANK `S` or port AT SEA/CRUISING; `is_turnaround` = RANK ends `T`. **Proven identical to the validated pipeline: 37,592 rows, 0 miss / 0 extra.**
  - **Azamara long** if header row has `Ship` + `Location` + `Cruise Nr`. Brand forced = Azamara; `ship_short` = Ship minus the `Azamara ` prefix (handles name changes). `is_sea` = Location/Country `At sea`.
    - **AZAMARA TURNAROUND RULE (taught by Miguel 2026-07-08 — DO NOT change):** the **`Day`** column marks the cruise day. A Day cell that is **`1`** or **ends in `/1`** (e.g. `12/1` = day 12 of the ending cruise AND day 1 of the next) is the **crew-change turnaround** day. Cruise lengths vary (10/1, 16/1, 22/1…). The Day cell MUST be read as a **STRING** — `parseInt("12/1")` returns 12 and drops the `/1`, which was the original bug (marked turnarounds a day off via a cruise-change guess). Regex: `/(^|[/])1$/`. Yields ~2,961 rows / **261 turnarounds** across the 4 ships (Journey → Stockholm & Copenhagen, correct).
...
## Fleet health (audited 2026-07-08, both files loaded)
- All 48 fleet ships covered; every ship has 47+ turnarounds and ~2-year horizons — the picker always has valid sign-off ports.
- Only soft flag: Spectrum coverage ends 2027-05-01 (still > 6-month picker horizon). 6 non-fleet Celebrity future ships (Boundless/Compass/Roamer/Seeker/Wanderer, Flora) sit in the table harmlessly — keep them; they line up when those ships enter the fleet.
- **Known data note:** Azamara keyman `off_date`s in `ship_leg` (Keyman-board owned) can sit mid-cruise, NOT on a turnaround (e.g. Journey off Berlin Jul 25, real turnaround = Copenhagen Jul 27). Relievers should join at the real turnaround. Options: re-align the 4 Azamara off-dates to nearest turnaround (gated ship_leg write) or handle per-ship when assigning relievers.
