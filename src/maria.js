...
export const MARIA_GLOSSARY = [
  ...
  "- km (keyman_contract3.km): the CRUISE-LINE Ship's Crew ID of the crew on that leg (= crew.ship_crew_id when known) — NOT the agency id; the agency id on that table is the sc column.",
  ...
  "- keyman_contract3 (kc3) — the Contract Counter sheet of the CIMS Keyman workbook, the SECOND TDG import file (the one with sign-on / projected sign-off dates; AdvancedQuery has none). Since 2026-09-14 it is THE source of current legs for the board, status, the relief printers, the roster export, the crew card and the rank map (counter_legs.js is the one definition; a crew's latest contract is current; past its projected sign-off it is overdue, not gone). Refreshed only by a Counter upload (last 2026-07-06). Never a payout input.",
  "- contract_edit — the sign-on / sign-off / ship RITA RECORDED in the console for one contract. The NEWER WRITE WINS against keyman_contract3 (counter_sync.resolveLeg), so a date the board shows is often hers, NOT the Counter\'s. NEVER answer \'where does this date come from\' from keyman_contract3 alone: use crew_contract_history and read `resolved`, which carries the effective value, which side it came from, and both candidates. A Counter row imported before 2026-09-14 has no imported_at, which is why any recorded edit beats it AND why the console cannot name the TDG file behind it: report that as not recorded, never as a date TDG confirmed.",
  "- ship_leg — a frozen snapshot of the 2026-07-06 Contract Counter (source 'keyman_roster'). NOT a source of current legs any more: counter_legs.js reads it only as port memory for those snapshot rows and as the orphan arm (a snapshot leg whose crew has no Counter row). Projected forward legs (source 'assignment:%', is_current=0) live here too and never enter the schedule.",
  "- boardLegs() — the ONE schedule = Counter current legs + in-force assignment rows from the relief board (crew aboard per Rita). Status, the board, the movements email and the month view derive from it. One crew may hold legs on two ships (jumpers).",
  ...
  "",
  "QUERY GUIDANCE",
  "- Movements/arrivals/departures -> upcoming_movements. It reads the LIVE schedule (Contract Counter legs + the relief board), NOT ship_leg — never name ship_leg as the source of a current leg or a sign-off date.",
...
export const MARIA_TOOLS = [
  ...
  { name: "crew_contract_history", description: "Per-contract history for ONE crew member. `resolved` is AUTHORITATIVE and is what the Keyman board shows: effective ship + sign-on + sign-off per contract, `shown_from` (recorded in the console, or Contract Counter), and both candidate values under `counter` and `recorded`. `legs` is the raw Contract Counter rows — quote it only as the Counter's side. `sign_off_is_projected` true means the date is a PLAN, not a recorded sign-off. `counter.origin` says which TDG import the Counter row came from; when it says NOT RECORDED, say exactly that rather than implying the date has a known TDG file behind it. Provide agency_id or a name.", input_schema: { type: "object", properties: { name: { type: "string" }, agency_id: { type: "string" } }, additionalProperties: false } },
  ...
  { name: "upcoming_movements", description: "Upcoming crew movements from the LIVE rotation schedule: who signs ON (arrives/embarks) and OFF (departs/debarks) within the next N days, with vessel, port and date. THIS is the correct source for any 'who is debarking/arriving/joining/leaving soon' question. Do NOT use find_crew/contract_ledger sign-off dates for that — those are historical. Every row carries `confirmed`: false means the date is PROJECTED (the plan of record — TDG's projected sign-off or a relief projection), not a recorded movement. When you quote a date whose `confirmed` is false, say it is projected.", input_schema: { type: "object", properties: { days: { type: "number", description: "Look-ahead window in days; default 10" } }, additionalProperties: false } },
