...
export const MARIA_GLOSSARY = [
  ...
  "- ship_leg — TRUTH for rotation/board/status/month-billing/movements TOGETHER WITH assignment: the ONE schedule is boardLegs() = ship_leg rows with is_current=1 (a frozen snapshot of the 2026-07-06 Contract Counter, source 'keyman_roster') + in-force assignment rows from the relief board. Never read ship_leg alone for 'who is aboard'.",
  "- keyman_contract3 (kc3) — the Contract Counter sheet of the CIMS Keyman workbook, the SECOND TDG import file (the one with sign-on / projected sign-off dates; AdvancedQuery has none). Refreshed only by a Counter upload (last 2026-07-06); one imported contract per crew today. Read by: Score Card rank/contract count, the /api/daysworked range export (billing_range tool), sign-off flows by (sc,seq), the crew card. NOT the source of status, the board, or month billing (those read boardLegs).",
