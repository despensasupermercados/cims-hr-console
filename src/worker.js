...
function portOptions(ports,date,current){var ta=(ports||[]).filter(function(p){return Number(p.is_turnaround)===1&&Number(p.is_sea)!==1&&p.port_name;}).sort(function(a,b){return a.berth_date<b.berth_date?-1:a.berth_date>b.berth_date?1:0;});var start=0;if(date){for(var i=0;i<ta.length;i++){if(ta[i].berth_date>=date){start=i;break;}}}var win=ta.slice(Math.max(0,start),start+6);var opts='',seen={},matched=false;for(var j=0;j<win.length;j++){var p=win[j],v=String(p.port_name).replace(/"/g,'');if(seen[v])continue;seen[v]=1;var isSel=date?(p.berth_date===date):(current&&v===current);if(isSel)matched=true;opts+='<option value="'+v+'"'+(isSel?' selected':'')+'>'+p.port_name+' · '+p.berth_date+'</option>';}if(current&&!matched){opts='<option value="'+String(current).replace(/"/g,'')+'" selected>'+current+'</option>'+opts;}return opts||'<option value="">— no ports —</option>';}async function editContractModal(id,seq){
  ...
  var h='<div class=modcard><div class=modhd><div><div class=cname>Edit contract — '+e.name+'</div><div class=csub>'+id+' · contract #'+seq+'</div></div><button class="btn ghost" onclick="closeRotModal()">Close ✕</button></div>'
   ...
   +fld('Embark city · from itinerary','<select id=eEmb style="width:100%">'+portOptions(P,e.signOn,e.on_city||e.embark)+'</select>')
   +fld('Disembark city · from itinerary','<select id=eDis style="width:100%">'+portOptions(P,e.signOff,e.off_city||e.disembark)+'</select>')
