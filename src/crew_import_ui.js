...
async function apply(){
 ...
 $("msg").innerHTML='<div class="msg">&#10003; Applied '+res.applied+' changes · added '+res.added+' crew · '+res.open_conflicts+' flags for the board'+(function(f){var n=f?((f.closed_board_matches||0)+(f.closed_superseded||0)+(f.closed_dismissed||0)):0;return n?' · '+n+' earlier flag'+(n===1?'':'s')+' closed (board already matches, or superseded)':'';})(res.ship_flags)+' · logged to import history. '+(res.override_cleared?res.override_cleared+' manual entr'+(res.override_cleared===1?'y':'ies')+' replaced by the file'+(res.override_skipped?' ('+res.override_skipped+' changed since review, left alone)':'')+'.':'Nothing else was touched.')+'</div>';
}
