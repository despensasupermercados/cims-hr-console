// Maria knowledge — "Focus Drop" panel + AI auto-titling (Session-7).
...
test("Focus Drop panel: hero drop auto-ingests, no title field, result card rises", () => {
  assert.ok(src.includes("class=kbhero"), "hero drop zone");
  assert.ok(src.includes("function kbIngest"), "auto-ingest on drop/choose");
  assert.ok(src.includes("Maria is reading and naming it"), "naming progress state");
  assert.ok(src.includes("Maria named it"), "resolved confirmation");
  assert.ok(!src.includes("id=kbtitle"), "no manual title field");
  assert.ok(!src.includes("id=kbdate"), "no manual date field");
});
