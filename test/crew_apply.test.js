...
test("D3 override explicitly accepted -> base write + audit + that override field cleared", () => {
  ...
  assert.deepEqual(plan.overrideClears, [{ agency_id: "SC-1", field: "phone" }]);
});
...
test("status under a live override: accept clears crew_override.status so the card can change", () => {
  const review = { groups: { override_conflict: [{ agency_id: "SC-0038392", field: "status", old: "On board", new: "Inactive" }] } };
  ...
  assert.deepEqual(plan.overrideClears, [{ agency_id: "SC-0038392", field: "status" }]);
