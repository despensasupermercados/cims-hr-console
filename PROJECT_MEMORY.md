...
## Session — 2026-07-03/04: Shipboard Management Review — Phase A BUILT (branch, not merged)

**Naming (binding, Miguel):** "GSM" is RETIRED. Program = **Shipboard Management Review (SBM)**; respondent = **shipboard manager**. Brand-neutral so the same templates serve RCI / Celebrity / Azamara / future customers. Public route **cims.work/sbm** (single-use token). Spec: `docs/shipboard-review/SBM-REVIEW-REQUIREMENTS.md` (v2 — §6 is the authoritative money spec). Mockups (approved through several rounds): `docs/shipboard-review/sbm-survey-mockup.html`, `sbm-emails-mockup.html`. Drive backups: folder "Crew feedback GSM" id 1L_VA5ppEICP1FCYNXxb6uiNVfC3VMbNp.

**BUILD STATE — Phase A complete on branch `feat/shipboard-review` (head 416c00d, 5 commits). NOT merged; worker wiring NOT yet applied.**
- `migrations/0013_sbm_review.sql`: `sbm_review_request` (UNIQUE(agency_id, contract_signoff); status sent/reminded/submitted/expired/suppressed), append-only `sbm_review_response` (rating CHECK 1–5 + q_business…q_final), `sbm_config` (key/value). NO seval tables — those belong to Phase B (money PR).
- `src/sbm.js` (47KB, self-contained): tokens reuse the existing auth.js HMAC (`sbmToken`/`sbmVerify`, cross-purpose-rejecting); survey page = faithful port of approved mockup (no brand switcher — brand from request row); `sbmSubmit` (single-use, 409 on reuse); `sbmDailySweep` (T−7 invite, T−4 reminder ONCE, suppression matrix, expiry pass); `sbmCrewCards`; 4 email templates (invite/reminder signed Rita Berenyi Head of HR; internal full version To rita.berenyi@dg3.com cc miguel.sanmartin@dg3.com + `team_list`; crew-facing variant WITHOUT score/gate mechanics to `shipmail:<ship>`); wired via `installSbm(deps)` following the repo's installAck/installAutoSend pattern; mailer = existing cims-mailer path.
- `test/sbm.test.js`: 26 tests. Full suite **207/207 green** (baseline main = 181/181; NO existing test modified). Includes a guard test that nothing manager-facing mentions bonus/sEval/gate.
- **worker.js (318KB) cannot be pushed via the GitHub MCP** (payload too large for one tool call). The exact 5-hunk / 31-insertion diff (import + installSbm, `/sbm` + `/api/sbm/submit` public routes and `/api/sbm/crew` session route inside the error-boundary wrapper, `.catch`-guarded `sbmDailySweep` in scheduled(), crew-tab "Manager Feedback" section below Contract history) lives in `docs/shipboard-review/WORKER_WIRING.md` with grep anchors. Apply via GitHub web editor on the branch, then CI, then PR to main for Miguel. Known nit: the doc's `index` metadata line carries a placeholder post-apply hash — inert.

**Architecture decisions:** everything SBM in one module; sweep is DORMANT until `sbm_config` seeded (`recipient:<ship>` → fallback `recipient:<brand>` → skip+log); invite/reminder fire on the EXACT T−7/T−4 day (hourly cron retries within the day; a full-day outage skips that leg — no retroactive blast); crew identity is deterministic from the request row (never name-matching); responses append-only.

**sEval (Phase B, NOT built — money PR):** Score Card Supervisor Evaluation (1–5, 15%; 1–2 → bonus forfeited + count held; 3/4/5 → full 15) gets value+source `auto|manual|none`; auto = prefill only, badge "from shipboard review"; manual override by MONEY_USERS with mandatory reason; **manual always wins, auto never overwrites manual**; multiple reviews → average rounded half-up, raws in evidence; post-Commit arrivals: `bonus_outcome` immutable, card flagged "post-commit — no bonus effect"; every transition → append-only audit. Requires Miguel-approved PR per CLAUDE.md §1.

**Goals:** (G1) Miguel merges wiring PR after review → RCI pilot dormant-safe. (G2) Seed `sbm_config` (recipient list, team_list, shipmail) — awaiting Miguel. (G3) Phase B money PR (seval_state + seval_audit + Score Card auto-display/override + Commit reading the field). (G4) Celebrity/Azamara config (ship lists exist in VESSEL_REF; accents CEL #33415C / AZ #0E8C8C are placeholders). (G5) Decide historical MS Forms import → Manager Feedback cards (one completed RCI response exists as PDF sample). (G6) Console editor UI for sbm_config (currently seed via SQL).

**Open questions pending Miguel (asked 2026-07-04):** (Q1) apply worker wiring + open PR now? (Q2) missed-cron catch-up: keep exact-day-only or allow late send within N days? (Q3) import historical Forms responses? (Q4) recipient model he will supply: per-ship, per-brand, or mixed (code supports ship→brand fallback).
