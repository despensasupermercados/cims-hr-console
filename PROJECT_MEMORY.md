...
## Session — 2026-07-03/04: Shipboard Management Review — Phase A MERGED & LIVE

**Naming (binding, Miguel):** "GSM" is RETIRED. Program = **Shipboard Management Review (SBM)**; respondent = **shipboard manager**; route **cims.work/sbm**. Spec: `docs/shipboard-review/SBM-REVIEW-REQUIREMENTS.md` (v2; §6 = authoritative money spec). Approved mockups in same folder. Drive backups: folder "Crew feedback GSM" id 1L_VA5ppEICP1FCYNXxb6uiNVfC3VMbNp.

**STATE: PR #8 MERGED to main by Miguel's explicit instruction (merge commit 4a4de8b, 2026-07-04); deploy VERIFIED LIVE — https://cims.work/sbm serves the graceful expired-link page.** Suite **214/214** (baseline 181; no existing test touched). Branch contents: `migrations/0013_sbm_review.sql` (sbm_review_request UNIQUE(agency_id,contract_signoff); append-only sbm_review_response; sbm_config key/value), `src/sbm.js` (self-contained: auth.js HMAC tokens purpose "sbm", survey page = approved mockup, submit single-use 409, T−7/T−4 sweep + suppression, crew cards, 4 cims-mailer templates), `test/sbm.test.js` (33 tests), `docs/shipboard-review/WORKER_WIRING.md`. **worker.js wiring APPLIED** via GitHub web editor (blob `7bf03a6` = byte-identical to locally tested file): routes /sbm + /api/sbm/submit (public) + /api/sbm/crew (session) inside the error boundary, `.catch`-guarded sweep in scheduled(), crew-tab "Manager Feedback" section below Contract history. Technique note: worker.js (318KB) exceeds MCP push limits — patch applied through github.com editor (select-all + insertText of pre-verified text), then blob-SHA-verified. OAuth token cannot push `.github/workflows/*` (workflow scope missing).

**Miguel's decisions (2026-07-04, recorded on PR #8):** green light given ("wire it and open the PR"); **exact-day T−7/T−4 only, NO catch-up** (his explicit choice over auto_send's window model — revisit before pilot if late schedule data proves common); **historical MS Forms import = YES** (one-time, separate PR when Miguel exports the CSV); **recipients = one address per ship** (list pending from Miguel).

**Adversarial review (commit 48cb087 fixed):** S1 ship-key normalization — sbm_config keys accept board names OR canonical short names (`sbmNormShip`; sections emit "Navigator", humans write "Navigator of the Seas"; without this the sweep skipped everything silently); S3 logActivity + console.error on send failures / missing recipients + per-run summary; S4 sweep gated to 08:00 Europe/Budapest like auto_send; N5 ensureSbm DDL = migration DDL; N6 malformed sign-off date can't mint a non-expiring token; N7 crew-cards error shape aligned (HTTP status deferred, worker.js frozen). **Accepted/open:** internal notification ③ has no outbox fallback if MAILER binding absent (N1); **crew-facing copy currently shows the numeric rating immediately incl. 1–2 (N2) — ASK MIGUEL: keep, or gate crew copy on rating ≥3**; no console editor for sbm_config yet (G6, seed via SQL).

**Money rules (Phase B, NOT built — separate Miguel-approved PR):** sEval value+source auto|manual|none; auto = prefill only; manual (MONEY_USERS, mandatory reason) ALWAYS wins; multiple reviews → average rounded half-up; post-Commit arrivals never touch immutable bonus_outcome (flag "post-commit — no bonus effect"); append-only seval audit. Nothing writes bonus_outcome except Commit.

**Post-merge checklist:** (1) migration 0013 staging → prod; (2) verify live /sbm, crew-tab section, `sbm_sweep` cron logs; (3) seed sbm_config (`recipient:<ship>` per-ship list from Miguel, `team_list`, `shipmail:<ship>`); (4) Forms-history import PR; (5) Phase B money PR; (6) G6 config editor UI; (7) confirm CEL #33415C / AZ #0E8C8C accents before those brands go live.

**sEval default rule (Miguel, 2026-07-04 — supersedes 'none' source):** Supervisor Evaluation is NEVER empty: defaults to **3** for every contract (neutral, full 15 pts). Survey rating replaces the default (`auto`); Rita/Miguel manual override (with reason) wins over everything and is never overwritten by later surveys. Value at Commit feeds the bonus. Spec §6.1 updated accordingly.

**Onboard roles taxonomy (KEEP — reused constantly):** shipboard manager = **GSM** (Guest Services Manager, all Royal Caribbean) / **BOM** (Business Operations Manager, all Azamara) / **RDM** (Revenue Marketing Director, Celebrity seafarers). **Crew Admin Manager** + **Crew Admin** = onboard sign-on/sign-off logistics, NOT review recipients. Per-ship recipient email list: Miguel intended to attach it 2026-07-04 but the attachment did not come through — STILL PENDING; sbm_config remains unseeded, sweep dormant.

**Historical import prep (2026-07-04):** Miguel's xlsx (37 Forms responses, Jun 2025–Jun 2026, 21 ships, ratings 6×3/25×4/6×5, all qualitative fields filled) matched against prod crew table: 32/37 rows auto-matched ≥0.95 to agency_ids; 5 need Miguel: #2 'Miguel San Martin' on Icon (test row — propose exclude), #9 bare 'Raymond' on Allure 2025-09-17 (Quiming SC-0045989 vs Imperial SC-0045444; keyman inconclusive), #17 'Mandrinico, Rommel' (typo — propose SC-0038368), #18 'Azariah Asim' Radiance (no roster match), #24 bare 'Baris' Utopia (no roster match). Match table: outputs file sbm-history-match.csv (shared with Miguel + to be re-generated at import). Import PR waits for Miguel's confirmations. NOTE: 68% of historical ratings are exactly 4 — anchoring bias from the old form's example, baseline for future comparisons.

### Fleet email roster (2026-07-04) — WHERE IT LIVES (no addresses here: repo is public, roster is third-party PII)

Miguel supplied "Email addresses - Fleet.xlsx". **The addresses themselves are stored ONLY in private locations:** (1) Miguel's Google Drive folder "Crew feedback GSM" (id 1L_VA5ppEICP1FCYNXxb6uiNVfC3VMbNp): `fleet-emails-normalized.csv` (file id 1PRXXreV7F8atC0uKVhbGtkvRdl94SmTr, 188 records: brand/ship/role/email) and `sbm-config-seed.sql` (file id 1M6Bcno5ZZ1NUyeU24L8tFZ52Z0BEpFqD, 96 INSERT OR REPLACE rows: 48 `recipient:` + 48 `shipmail:`); (2) once Miguel gives the go, seeded into prod D1 `sbm_config`. NEVER paste these addresses into this repo, PROJECT_MEMORY, or any public artifact.

**Coverage facts (safe to record):** 48 ships total, supervisor + crew-admin + printer-specialist working address = 100% coverage. RCI 30 ships (all 29 expected + **MV LEGEND OF THE SEAS — extra, confirm with Miguel**), Azamara 4/4 (BOM; Crew Admin Manager column structurally blank on AZ), Celebrity 14 (Apex Ascent Beyond Constellation Eclipse Edge Equinox Infinity Millennium Reflection Silhouette Solstice Summit Xcel; Galapagos ships absent). All addresses are positional role accounts (`<code>_gsm@…`, `_pm@`/`_printerspecialist@`/`_printer@` for the specialist's ship mailbox → these feed `shipmail:<ship>` for the crew-facing copy). Ship keys in the roster are "MV <NAME>" uppercase — the sbm lookup's normalizer handles this. Celebrity printer addresses use a different domain (celebritycruises.com) than its other roles (celebrity.com) — one-time deliverability check advised.
