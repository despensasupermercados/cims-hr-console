...
## Session — 2026-07-03: GSM Feedback Survey (requirements + mockups v3; AWAITING GREEN LIGHT)

**What this is.** Replacing the Microsoft Forms "Crew Feedback Survey – Printer Specialist" with a CIMS-native survey at **cims.work/gsm**, auto-sent to the ship's Guest Services Manager before a specialist signs off. Miguel's instruction: mock up first, iterate, integrate ONLY after his explicit green light. Mockups v3 delivered after two feedback rounds; NOT yet integrated.

**Workflow (agreed):** invite email at T−7 before seafarer sign-off; ONE gentle reminder at T−4 if unanswered; on submission an INTERNAL notification email to Rita (cc Miguel, money users only) with score, gate status, and Score Card link; suppress invite/reminder if crew signed off / already submitted / contract cancelled. Single-use signed token link (same pattern as /fb), no GSM login. Sender `CIMS <cims@cims.work>` via cims-mailer. Rollout: Royal Caribbean first, then Celebrity Cruises and Azamara variants (per-brand supervisor email lists still pending from Miguel).

**Miguel's feedback (applied through v3):** brand chip naming the customer — keep. Specialist shown as prominent name block — NO avatar circle (removed on request), NO anchor symbol. "Date of review" field removed — submission timestamp captured server-side. "Details wrong? let us know" → `mailto:rita.berenyi@dg3.com`. ALL emails signed "With appreciation, Rita Berenyi / Head of HR / DG3 Cruise Industry Managed Services" (not Miguel). Emails given more whitespace and stacked vertically in the mockup file (side-by-side hid the reminder). **Logo:** the estate brand icon (navy squircle + green waves — same asset as cims.work favicon, `src/icons.js`) reproduced as INLINE SVG in the survey header and all three email headers; live-URL <img> and green-"D" approaches were both rejected. On navy headers the tile uses a slightly lighter fill (#24486E) + faint white stroke so it stays visible.
...
**Artifacts:** `docs/gsm-survey/` in this repo (requirements + survey + 3-email mockups, v3). Backups in Miguel's Google Drive folder "Crew feedback GSM" (id 1L_VA5ppEICP1FCYNXxb6uiNVfC3VMbNp) — v1/v2 copies there are superseded by v3. Mockup brand switcher accents: RCI #1E6FD0 / Celebrity #33415C / Azamara #0E8C8C — CEL/AZ are placeholders, confirm at integration. Standard CIMS tokens (navy #1B3A5C, green #5FB946, Outfit/DM Sans).
