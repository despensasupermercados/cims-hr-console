...
## Session — 2026-07-03: Shipboard Management Review (requirements v2 + mockups; AWAITING GREEN LIGHT)

**Naming (binding, Miguel):** the term "GSM" is RETIRED everywhere. Program = **Shipboard Management Review** (SBM review); respondent = **shipboard manager**. Brand-neutral so the same templates serve RCI / Celebrity / Azamara / future customers. Public route: **cims.work/sbm** (single-use token, /fb pattern). Repo artifacts moved to `docs/shipboard-review/` (old `docs/gsm-survey/` deleted).

**What this is.** CIMS-native replacement for the MS Forms "Crew Feedback Survey – Printer Specialist", auto-triggered from the HR console around each specialist's sign-off. Mockups (survey page + 3-email set) approved through several feedback rounds; full spec in `docs/shipboard-review/SBM-REVIEW-REQUIREMENTS.md`. NOT integrated — Miguel's explicit green light required first.

**Workflow:** ① invite T−7 before sign-off to shipboard manager · ② ONE gentle reminder T−4 if unanswered · ③ internal notification on submission. Suppress ①/② if signed off / submitted / date out of window / contract cancelled / retired. No response by sign-off = Score Card manual fallback, a missed review never zeroes a bonus. Sender `CIMS <cims@cims.work>`; ①/② signed by Rita Berenyi, Head of HR.

**sEval integration (THE money rules — spec §6, follow exactly):** Score Card Supervisor Evaluation (1–5, 15%; 1–2 → bonus forfeited + count held; 3/4/5 → full 15) gets value + source `auto|manual|none`. Auto = from review, PREFILL ONLY, badge "from shipboard review". Manual = money-user override with mandatory reason; **manual always wins; auto never overwrites manual**. Multiple reviews per contract: average, rounded half-up, raws in evidence panel. Review after Commit: `bonus_outcome` immutable, response files to crew card flagged "post-commit — no bonus effect". Every value/source transition → append-only audit (who/when/old→new/reason). Nothing writes `bonus_outcome` except Commit by money users. Wiring this is a money change → PR + Miguel approval. Keep fully separate from crew-intel (never-money).

**Crew record (Miguel docx):** crew tab, below Contract History → new **"Manager Feedback"** section; one card per response (ship, brand, contract, rating, verbatim answers, date). Deterministic attach via token's crew/contract ID — no name matching. Append-only, permanent.

**Notification ③ distribution (Miguel docx):** To Rita, cc Miguel + CIMS team list (config). Also cc the specialist's WORKING SHIP EMAIL (never personal) with a **crew-facing variant: feedback content only, NO score/gate/bonus mechanics** — the full version with sEval/gate goes only to the internal list. Shipboard manager never sees bonus mechanics either.

**Survey content:** same 11 Forms questions; ship+name prefilled from token; review date captured server-side (field removed); only the 1–5 rating required; Q4 numeric anchor dropped (bias). Corrections link = mailto:rita.berenyi@dg3.com.

**Branding:** estate icon (navy squircle + green waves, src/icons.js) as inline SVG; wordmark "DG3 CIMS"; navy #1B3A5C / green #5FB946; Outfit/DM Sans. Accents: RCI #1E6FD0; CEL #33415C, AZ #0E8C8C placeholders.

**Artifacts:** `docs/shipboard-review/` (requirements v2, sbm-survey-mockup.html, sbm-emails-mockup.html). Backups in Miguel's Drive folder "Crew feedback GSM" (id 1L_VA5ppEICP1FCYNXxb6uiNVfC3VMbNp) — older gsm-* copies there are superseded.

**Open items for Miguel:** (1) per-brand/ship shipboard-manager email list; (2) team distribution list for ③; (3) import historical MS Forms responses as Manager Feedback cards (recommended yes, one-time CSV); (4) confirm route /sbm; (5) confirm CEL/AZ accents; (6) manager-rotation edge case; (7) **GREEN LIGHT** → build order: D1 tables → /sbm route → scheduler → mailer templates (incl. crew-facing variant) → crew-tab section → Score Card auto/override (money PR) → RCI pilot → CEL/AZ config.
